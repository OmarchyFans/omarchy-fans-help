#!/bin/bash
# GPU bake-off. Generated from bench.sh; -ngl 99 offloads what fits in the
# RTX 3050 Ti (3.7GB usable). The 27B is 14GB so most layers stay on CPU --
# that partial-offload number is the point of including it.
#
...
OUT="$HOME/.local/share/omarchy-local-agent/bench-results-gpu.txt"
...
# The GPU has 4GB and the production llama-server holds ~3.2GB of it. Running
# the bake-off against a live service silently fails EVERY model with an
# allocation error that reads as "FAILED to benchmark" -- which is exactly what
# happened on the first attempt. Refuse to start rather than produce a log full
# of failures that look like model problems.
if systemctl --user is-active --quiet omarchy-local-agent; then
  echo "ABORT: omarchy-local-agent is running and holding VRAM." >&2
  echo "  systemctl --user stop omarchy-local-agent" >&2
  echo "  bash $0" >&2
  echo "  systemctl --user start omarchy-local-agent" >&2
  exit 1
fi
used=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits 2>/dev/null || echo 0)
if [ "${used:-0}" -gt 500 ]; then
  echo "ABORT: ${used} MiB of VRAM already in use; results would be wrong or fail." >&2
...
for m in "$MODELS"/*.gguf; do
  ...
  if ! llama-bench -m "$m" -t "$THREADS" -ngl 99 -p 512 -n 128 -r 2 2>&1 \
...
prose_smoke() {
  ...
  setsid llama-server --model "$model" --host 127.0.0.1 --port "$PORT" \
      --ctx-size 8192 --threads "$THREADS" --n-gpu-layers 99 --no-webui >/dev/null 2>&1 &
  local pid=$!
