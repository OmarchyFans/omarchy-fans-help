...
This is the reference implementation (0.3.1, PR #4, squash commit 970e523).

- Kind: panel (`HelpPanel.qml`, a floating window) plus a bar widget; helper `bin/omarchy-local-agent` (Python), copied into `~/.local/bin` by `install.sh`.
- Helper flags: `--update-check [PANEL_VERSION] [--force]`, `--update-dismiss VERSION`, `--update-run [all|install]`; the code sits under the "updates:" heading in the helper, the banner under "update banner" in `HelpPanel.qml`.
- Version places: `manifest.json`, the `VERSION` constant in `bin/omarchy-local-agent`, and `CHANGELOG.md`, bumped together.
- Cache: `~/.cache/omarchy-local-agent/update-check.json`. Opt-out: `"update_check": false` in `~/.config/omarchy-local-agent/config.json`.
- Mismatch case: the helpers are copies, so `omarchy plugin update` leaves them old. The panel sends its own version; a helper that reports a different version, or one too old to know the flag (it exits 2), makes the panel show *Finish update…*, which runs `install.sh`.
- The other Omarchy.Fans plugins carry the same design in a shared bash `lib/update.sh`; this repo keeps its Python original.
- The marketplace card shows only one clipped line of `manifest.description`, so lead with what the plugin does, not with the update feature.
